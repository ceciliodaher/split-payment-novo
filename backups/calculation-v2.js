 ...
 */
function calcularFluxoCaixaDescontadoAjustado(dados, anoInicial, anoFinal, parametrosAdicionais) {
...
function calcularImpactoCicloFinanceiro(dados, ano = 2026, parametrosSetoriais = null) {
    ...
    // CORREÇÃO: O impacto no ciclo financeiro é diferente do que estava sendo calculado anteriormente
    // No regime atual, o valor do imposto fica disponível como capital de giro por um período
    // No split payment, parte desse valor é retido imediatamente
    
    // Dias de capital de giro disponível no regime atual (assumindo recolhimento no dia 25 do mês seguinte)
    const diasCapitalGiroAtual = 25 + pmr;
    
    // O valor split não fica disponível como capital de giro
    // O valor retido como percentual do faturamento
    const percentualRetido = (impostoSplit / faturamento);
    
    // Aumento da necessidade de capital de giro em dias de faturamento
    const impactoDiasCapitalGiro = diasCapitalGiroAtual * percentualRetido;
    
    // CORREÇÃO: O ciclo financeiro AUMENTA devido ao split payment, não diminui
    const cicloFinanceiroAjustado = cicloFinanceiroAtual + impactoDiasCapitalGiro;
    ...
    return {
        ...
        memoriaCritica: {
            ...
            formulaImpactoDiasCapitalGiro: `Impacto em Dias de Capital de Giro = Dias de Capital Disponível Atual × Percentual Retido = ${diasCapitalGiroAtual} × ${(percentualRetido * 100).toFixed(2)}% = ${impactoDiasCapitalGiro.toFixed(2)} dias`,
            formulaCicloAjustado: `Ciclo Financeiro Ajustado = Ciclo Atual + Impacto em Dias = ${cicloFinanceiroAtual} + ${impactoDiasCapitalGiro.toFixed(2)} = ${cicloFinanceiroAjustado.toFixed(2)} dias`,
            ...
            observacoes: [
                `O Split Payment aumenta o ciclo financeiro em ${impactoDiasCapitalGiro.toFixed(2)} dias.`,
                `A necessidade de capital de giro é aumentada em ${formatarMoeda(diferencaNCG)}.`,
                `Este aumento ocorre porque o valor dos impostos, que antes ficava disponível como capital de giro até o recolhimento, agora é retido imediatamente no percentual de ${(percentualImplementacao * 100).toFixed(1)}% da alíquota total.`
            ]
        }
    };
}
