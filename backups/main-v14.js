...
function inicializarEventosPrincipais() {
     ...
     */
    // Substituir a implementação atual da função obterPercentualImplementacao
	function obterPercentualImplementacao(ano, parametrosSetoriais = null) {
		...
		if (parametrosSetoriais && 
			...
			typeof parametrosSetoriais.cronograma[ano] !== 'undefined') {
			return parametrosSetoriais.cronograma[ano];
		}
		
		// Caso contrário, utilizar o cronograma padrão ou valor padrão para ano não encontrado
...
// Adicionar após a função inicializarEventosPrincipais() no arquivo main.js

/**
 * Limpa todos os campos do formulário de simulação
 */
function limparFormularioSimulacao() {
    console.log('Limpando formulário de simulação');
    ...
    // Resetar campos numéricos para valores padrão
    const camposNumericos = [
        {id: 'margem', valor: '15'},
        {id: 'pmr', valor: '30'},
        {id: 'pmp', valor: '30'},
        {id: 'pme', valor: '30'},
        {id: 'ciclo-financeiro', valor: '30'},
        {id: 'perc-vista', valor: '30'},
        {id: 'perc-prazo', valor: '70%'},
        {id: 'aliquota', valor: '26.5'},
        {id: 'reducao', valor: '0.0'},
        {id: 'taxa-crescimento', valor: '5'}
    ];
    ...
    // Redefinir datas para valores padrão
    const campoDataInicial = document.getElementById('data-inicial');
    const campoDataFinal = document.getElementById('data-final');
    
    if (campoDataInicial) campoDataInicial.value = '2026-01-01';
    if (campoDataFinal) campoDataFinal.value = '2033-12-31';
    
    // Limpar área de resultados
    const areaResultados = document.getElementById('resultados');
    if (areaResultados) {
        areaResultados.innerHTML = '<p class="text-muted">Preencha os dados e clique em "Simular" para visualizar os resultados.</p>';
    }
    ...
    // Remover dados de simulação anteriores
    window.ultimaSimulacao = null;
    window.memoriaCalculoSimulacao = null;
    
    console.log('Formulário de simulação limpo com sucesso');
}

// Adicionar dentro da função inicializarEventosPrincipais() após outros event listeners
// Aproximadamente na linha 136
...
function gerarGraficos(resultados) {
    ...
    // Gráfico de capital de giro - CORREÇÃO: garantir que estamos mostrando o aumento da NCG
    ...
    if (ctxCapitalGiro) {
        window.graficos.capitalGiro = new Chart(ctxCapitalGiro.getContext('2d'), {
            type: 'doughnut',
            data: {
                labels: ['Capital de Giro Atual', 'Necessidade Adicional'],
                datasets: [{
                    data: [
                        resultados.impactoBase.resultadoAtual.capitalGiroDisponivel,
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: 'Impacto na Necessidade de Capital de Giro'
                    },
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                const value = context.raw;
                                return context.chart.data.labels[context.dataIndex] + ': R$ ' + value.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                            }
                        }
                    }
                }
    ...
    // Gráfico de projeção - CORREÇÃO: Garantir que estamos mostrando o aumento da NCG ao longo do tempo
    ...
    if (ctxProjecao) {
        ...
        const margensPorAno = anos.map(ano => 
        ...
        window.graficos.projecao = new Chart(ctxProjecao.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Necessidade Adicional de Capital (R$)',
                        data: impactosPorAno,
                    ...
                    {
                        label: 'Margem Operacional Ajustada (%)',
                        data: margensPorAno,
