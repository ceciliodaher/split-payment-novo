...
const ExportTools = {
    ...
    exportarParaPDF: function() {
        ...
        try {
            ...
            // Definir margens
            const margins = this.config.pdf && this.config.pdf.margins ? this.config.pdf.margins : {
                top: 25,
                right: 15,
                bottom: 25,
                left: 15
     ...
     * @param {Function|null} obterMemoriaCalculo - Função para obter a memória de cálculo
    ...
    _adicionarMemoriaCalculo: function(doc, obterMemoriaCalculo, pageCount) {
        // Adicionar cabeçalho
        doc.setFontSize(16);
        doc.text('Memória de Cálculo', 15, 30);

        let currentPositionY = 40;

        // Verificar se a função obterMemoriaCalculo está disponível
        if (typeof obterMemoriaCalculo !== 'function') {
            console.warn('Função obterMemoriaCalculo não definida');
            doc.setFontSize(12);
            doc.text('Memória de cálculo não disponível.', 15, currentPositionY);
            return currentPositionY + 10;
        }

        try {
            // Obter a memória de cálculo
            const memoriaTexto = obterMemoriaCalculo() || 'Memória de cálculo vazia.';
            ...
            const linhas = doc.splitTextToSize(memoriaTexto, 180);

            // Adicionar as linhas ao documento
            doc.text(linhas, 15, currentPositionY);

            // Atualizar posição Y
            currentPositionY += linhas.length * 4; // Espaçamento entre linhas
            ...
            doc.setFontSize(12);
            doc.text('Erro ao processar memória de cálculo.', 15, currentPositionY);
            currentPositionY += 10;
        }
    ...
    _adicionarCapa: function(doc, dados, pageCount) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text('R$ ' + (typeof dados.faturamento === 'number' ? dados.faturamento.toLocaleString('pt-BR') : 'N/A'), margins.left + 40, 155);
        ...
        doc.text('Página ' + pageCount + ' de {totalPages}', doc.internal.pageSize.width - 45, doc.internal.pageSize.height - 15);

        // Adicionar logo (opcional)
        try {
            if (this.config.pdf && this.config.pdf.logoEnabled && this.config.pdf.logoPath) {
                doc.addImage(this.config.pdf.logoPath, 'PNG', margins.left, 20, 
                             this.config.pdf.logoSize.width, this.config.pdf.logoSize.height);
            } else {
    ...
    _adicionarIndice: function(doc, pageCount) {
        ...
        indiceItems.forEach(item => {
            ...
            const textWidth = doc.getStringUnitWidth(item.texto) * doc.internal.getFontSize() / doc.internal.scaleFactor;
    ...
    _adicionarCabecalhoRodape: function(doc, totalPages) {
        ...
        for (let i = 2; i <= totalPages; i++) {
            ...
            doc.text('Página ' + i + ' de ' + totalPages, doc.internal.pageSize.width - 45, doc.internal.pageSize.height - 15);
        }
    },
    ...
    _adicionarParametrosSimulacao: function(doc, dados, pageCount) {
        ...
        const dadosEmpresa = [
            ...
            { label: 'Faturamento Mensal:', valor: 'R$ ' + (typeof dados.faturamento === 'number' ? dados.faturamento.toLocaleString('pt-BR') : 'N/A') },
            { label: 'Margem Operacional:', valor: (dados.margem || 0) + '%' }
        ];
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(46, 204, 113); // Cor secundária
        doc.text('1.2. Tributação e Split Payment', margins.left, currentPositionY);
        currentPositionY += 10;
        ...
        const dadosTributacao = [
            { label: 'Alíquota Efetiva:', valor: (dados.aliquota || 0) + '%' },
            { label: 'Redução Especial:', valor: (dados.reducao || 0) + '%' },
            { label: 'Tipo de Operação:', valor: dados.tipoOperacao || 'N/A' },
            { label: 'Créditos Tributários:', valor: 'R$ ' + (typeof dados.creditos === 'number' ? dados.creditos.toLocaleString('pt-BR') : 'N/A') },
        ...
        // Seção Ciclo Financeiro
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(46, 204, 113); // Cor secundária
        doc.text('1.3. Ciclo Financeiro', margins.left, currentPositionY);
        currentPositionY += 10;

        // Adicionar dados do ciclo financeiro
        const dadosCiclo = [
            ...
            { label: 'Vendas à Vista:', valor: (dados.percVista || 0) + '%' },
            { label: 'Vendas a Prazo:', valor: (dados.percPrazo || 0) + '%' }
        ];
        ...
        const parametrosSimulacao = [
            { label: 'Data Inicial:', valor: dados.dataInicial ? this._formatarDataSimples(new Date(dados.dataInicial)) : 'N/A' },
            { label: 'Data Final:', valor: dados.dataFinal ? this._formatarDataSimples(new Date(dados.dataFinal)) : 'N/A' },
            { label: 'Cenário de Crescimento:', valor: dados.cenario || 'N/A' },
            { label: 'Taxa de Crescimento:', valor: (dados.taxaCrescimento || 0) + '% a.a.' }
        ];
     ...
     */
    _adicionarResultadosSimulacao: function(doc, resultados, aliquotasEquivalentes, pageCount) {
        // Configurações básicas da página
        const margins = this.config.pdf && this.config.pdf.margins ? this.config.pdf.margins : {
            top: 25,
            right: 15,
        ...
        // Cabeçalho da tabela
        const headers = ['Ano', 'Imposto Devido', 'Sistema Atual', 'Diferença', 'Variação (%)'];
        const colWidths = [30, 40, 40, 40, 35];

        // Desenhar cabeçalho
        doc.setFillColor(52, 152, 219);
        doc.setDrawColor(52, 152, 219);
        doc.setTextColor(255, 255, 255);
        doc.rect(margins.left, currentPositionY, doc.internal.pageSize.width - margins.left - margins.right, 10, 'F');

        let currentX = margins.left + 5;
        headers.forEach((header, index) => {
            doc.text(header, currentX, currentPositionY + 7);
        ...
        anos.forEach(ano => {
            ...
            const percentual = valorAtual !== 0 ? (diferenca / valorAtual) * 100 : 0;
            ...
            const formatPerc = (valor) => valor.toFixed(2) + '%';

            // Adicionar dados
            currentX = margins.left + 5;

            doc.text(ano, currentX, currentPositionY + 7);
            currentX += colWidths[0];

            doc.text(FormatacaoHelper.formatarMoeda(diferenca), currentX, currentPositionY + 7);
            currentX += colWidths[1];

            doc.text(FormatacaoHelper.formatarMoeda(diferenca), currentX, currentPositionY + 7);
            currentX += colWidths[2];

            // Usar cor diferente para diferença positiva/negativa
            if (diferenca > 0) {
                doc.setTextColor(231, 76, 60); // Cor para aumento (vermelho)
            } else {
                doc.setTextColor(46, 204, 113); // Cor para redução (verde)
            }

            doc.text(formatMoeda(diferenca), currentX, currentPositionY + 7);
            currentX += colWidths[3];

            doc.text(formatPerc(percentual), currentX, currentPositionY + 7);
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(46, 204, 113); // Cor secundária
        doc.text('2.2. Análise dos Resultados', margins.left, currentPositionY);
        currentPositionY += 10;
        ...
        const linhas = [
            `A variação total acumulada no período de ${anos[0]} a ${anos[anos.length - 1]} foi de ${formatMoeda(variacaoTotal)}.`,
            `O ano com maior impacto foi ${maiorImpacto.ano}, com uma diferença de ${formatMoeda(maiorImpacto.valor)}.`,
            `O ano com menor impacto foi ${menorImpacto.ano}, com uma diferença de ${formatMoeda(menorImpacto.valor)}.`
        ];
        ...
        const conclusao = variacaoTotal > 0 
            ? 'A implementação do Split Payment resultará em um aumento da necessidade de capital de giro.'
            : 'A implementação do Split Payment poderá resultar em uma redução da necessidade de capital de giro.';

        doc.text(conclusao, margins.left, currentPositionY);
        currentPositionY += 8;

        const recomendacao = 'Recomenda-se a análise das estratégias de mitigação apresentadas na seção 4 deste relatório.';
        doc.text(recomendacao, margins.left, currentPositionY);
    ...
    _adicionarGraficos: function(doc, pageCount) {
        ...
        try {
            ...
            graficos.forEach((grafico, index) => {
                ...
                if (currentPositionY > doc.internal.pageSize.height - margins.bottom - 30 && index < graficos.length - 1) {
                    doc.addPage();
                    currentPositionY = margins.top;
                }
            });
            ...
            doc.setFontSize(12);
            doc.setFont('helvetica', 'italic');
            doc.text('Gráficos não disponíveis. Verifique se os gráficos foram gerados antes de exportar.', margins.left, currentPositionY);
            currentPositionY += 20;
        }
    ...
    _adicionarAnaliseEstrategias: function(doc, dados, resultados, pageCount) {
        ...
        if (!window.resultadosEstrategias) {
            doc.setFontSize(12);
            doc.setFont('helvetica', 'italic');
            doc.text('Não há dados de estratégias disponíveis. Realize uma simulação de estratégias antes de exportar.', margins.left, currentPositionY);
        ...
        try {
            ...
            const linhasImpacto = [
                ...
                `Impacto Percentual: ${formatPerc(impacto.percentualImpacto/100)}`,
            ...
            estrategias.forEach((estrategia, index) => {
                ...
                if (dadosEstrategia) {
                    // Exibir efetividade
                    doc.setFont('helvetica', 'bold');
                    doc.text(`Efetividade: ${formatPerc(dadosEstrategia.efetividadePercentual/100)}`, margins.left + 10, currentPositionY);
                    currentPositionY += 8;

                    // Exibir detalhes específicos de cada estratégia
                    switch (codigoEstrategia) {
                        case 'ajustePrecos':
                            doc.text(`Fluxo de Caixa Adicional: ${formatMoeda(dadosEstrategia.fluxoCaixaAdicional || 0)}`, margins.left + 10, currentPositionY);
                            currentPositionY += 8;
                            doc.text(`Custo da Estratégia: ${formatMoeda(dadosEstrategia.custoEstrategia || 0)}`, margins.left + 10, currentPositionY);
                            break;
                        case 'renegociacaoPrazos':
                            doc.text(`Impacto no Fluxo de Caixa: ${formatMoeda(dadosEstrategia.impactoFluxoCaixa || 0)}`, margins.left + 10, currentPositionY);
                            currentPositionY += 8;
                            doc.text(`Custo Total: ${formatMoeda(dadosEstrategia.custoTotal || 0)}`, margins.left + 10, currentPositionY);
                            break;
                        case 'antecipacaoRecebiveis':
                            doc.text(`Impacto no Fluxo de Caixa: ${formatMoeda(dadosEstrategia.impactoFluxoCaixa || 0)}`, margins.left + 10, currentPositionY);
                            currentPositionY += 8;
                            doc.text(`Custo Total: ${formatMoeda(dadosEstrategia.custoTotalAntecipacao || 0)}`, margins.left + 10, currentPositionY);
                            break;
                        case 'capitalGiro':
                            doc.text(`Valor Financiado: ${formatMoeda(dadosEstrategia.valorFinanciamento || 0)}`, margins.left + 10, currentPositionY);
                            currentPositionY += 8;
                            doc.text(`Custo Total: ${formatMoeda(dadosEstrategia.custoTotalFinanciamento || 0)}`, margins.left + 10, currentPositionY);
                            break;
                    }
                } else {
                    doc.setFont('helvetica', 'italic');
                ...
                if (currentPositionY > doc.internal.pageSize.height - margins.bottom - 30 && index < estrategias.length - 1) {
                    doc.addPage();
                    currentPositionY = margins.top;
                }
            });
            ...
            const linhasCombinado = [
                `Efetividade Total: ${formatPerc(combinado.efetividadePercentual/100)}`,
    ...
    _adicionarConclusao: function(doc, dados, resultados, pageCount, aliquotasEquivalentes) {
        // Verificação inicial - usar objeto vazio se aliquotasEquivalentes não estiver definido
        ...
        anos.forEach(ano => {
            const resultado = resultados[ano];
            const valorAtual = (aliquotasEquivalentes && aliquotasEquivalentes[ano] && 
        ...
        const conclusaoTexto = `A implementação do Split Payment, conforme simulação realizada para ${dados.empresa || 'a empresa'}, 
    resultará em um ${variacaoTotal > 0 ? 'aumento' : 'decréscimo'} estimado de R$ ${Math.abs(variacaoTotal).toLocaleString('pt-BR', {minimumFractionDigits: 2})} 
    na necessidade de capital de giro durante o período de ${anos[0]} a ${anos[anos.length - 1]}.`;
    ...
    e sua necessidade de capital de giro.`;
        ...
        const linhasImpacto = doc.splitTextToSize(impactoTexto, doc.internal.pageSize.width - margins.left - margins.right);
    ...
    do Split Payment, que ainda está em fase de definição pelos órgãos competentes.`
        ...
        recomendacoes.forEach(recomendacao => {
            const linhasRecomendacao = doc.splitTextToSize(recomendacao, doc.internal.pageSize.width - margins.left - margins.right);
    ...
    à medida que novas informações forem divulgadas pelos órgãos competentes.`;
        ...
        const linhasConsideracoes = doc.splitTextToSize(consideracoesFinais, doc.internal.pageSize.width - margins.left - margins.right);
     ...
     */
     _criarPlanilhaResumo: function(dados, resultados, aliquotasEquivalentes) {
        ...
        // Garantir que this.config.colors existe
        if (!this.config || !this.config.excel || !this.config.excel.colors) {
            this.config = this.config || {};
            this.config.excel = this.config.excel || {};
